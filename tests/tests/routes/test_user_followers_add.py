"""
PUT /users/:username/followers/:follower route tests.
"""
...
async def test_keycloak_network_error(
    ...
    resp = await cli_no_kc_and_redis.put("/users/username/followers/follower", headers=headers)
...
async def test_redis_network_error(
    ...
    # Try to add a follower
    headers = data_generator.auth.get_bearer_header(access_token)
    resp = await cli_no_redis.put("/users/other_username/followers/username", headers=headers)
...
async def test_validation(
    ...
    # Follower validation
    resp = await cli_no_redis.put("/users/username/followers/a", headers=headers)
    assert resp.status_code == 422
...
async def test_invalid_token(
    ...
    headers = data_generator.auth.get_bearer_header_with_invalid_token()
    resp = await cli_no_redis.put("/users/username/followers/follower", headers=headers)
...
async def test_token_of_another_user(
    ...
    headers = data_generator.auth.get_bearer_header(access_token)
    resp = await cli.put("/users/some_user/followers/second_user", headers=headers)
...
async def test_non_existing_username(
    ...
    headers = data_generator.auth.get_bearer_header(access_token)
    resp = await cli.put("/users/non-existing/followers/username", headers=headers)

    assert resp.status_code == 404


async def test_self_following(
        data_generator: DataGenerator,
        keycloak_admin_client: KeycloakAdminClient,
        redis_admin_client: RedisAdminClient,
        cli: AsyncClient
    ...
    # Try to follow self
    headers = data_generator.auth.get_bearer_header(access_token)
    resp = await cli.put("/users/username/followers/username", headers=headers)

    assert resp.status_code == 400


async def test_add_followers_followers_set(
        data_generator: DataGenerator,
        keycloak_admin_client: KeycloakAdminClient,
        redis_admin_client: RedisAdminClient,
        cli: AsyncClient
    ...
    # Add users
    followed, first_follower, second_follower = "followed", "first_follower", "second_follower"
    ...
    # Log in & follow with different users
    for follower in (first_follower, second_follower):
        ...
        # Follow a user
        headers = data_generator.auth.get_bearer_header(access_token)
        resp = await cli.put(f"/users/{followed}/followers/{follower}", headers=headers)

        assert resp.status_code == 200
    
    # Check if followed user has correct followers
    assert redis_admin_client.get_user_followers(followed) == [first_follower, second_follower]


async def test_add_followers_follower_feed(
        data_generator: DataGenerator,
        keycloak_admin_client: KeycloakAdminClient,
        redis_admin_client: RedisAdminClient,
        cli: AsyncClient
    ...
    # Add users
    follower, first_followed, second_followed, not_followed = "follower", "first_followed", "second_followed", "not_followed"
    ...
    redis_admin_client.add_post(data_generator.posts.post(post_id=3, author=not_followed))
    ...
    redis_admin_client.add_post(data_generator.posts.post(post_id=6, author=not_followed))
