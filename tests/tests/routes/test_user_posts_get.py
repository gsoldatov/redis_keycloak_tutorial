"""
GET /users/:username/posts route tests.
"""
...
async def test_redis_network_error(
    ...
    resp = await cli_no_redis.get("/users/username/posts")
...
async def test_validation(
    ...
    # Username validation
    resp = await cli_no_redis.get("/users/a/posts")
    assert resp.status_code == 422

    # Last viewed validation
    resp = await cli_no_redis.get("/users/username/posts", params={"last_viewed": -1})
    assert resp.status_code == 422


async def test_posts_of_a_non_existing_user(
    cli: AsyncClient
):
    # Get posts without offset
    resp = await cli.get("/users/username/posts")
...
async def test_user_without_posts(
...
async def test_user_with_posts(
    data_generator: DataGenerator,
    redis_admin_client: RedisAdminClient,
    cli: AsyncClient
):
    ...
    # Add user's posts
    posts = [data_generator.posts.post(post_id=i) for i in range(1, 11)]
    for post in posts:
