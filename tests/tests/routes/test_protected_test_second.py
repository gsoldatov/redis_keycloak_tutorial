...
async def test_keycloak_network_error(
    ...
    resp = await cli_no_keycloak.get("/protected_test/second", headers=headers)
...
async def test_missing_authorization_header(
    ...
    resp = await cli_no_kc_and_redis.get("/protected_test/second")
...
async def test_invalid_authorization_header_format(
    ...
    resp = await cli_no_kc_and_redis.get("/protected_test/second", headers=headers)
    assert resp.status_code == 401
...
async def test_expired_token(
    ...
    route_resp = await cli.get("/protected_test/second", headers=headers)
...
async def test_expired_token_with_redis_network_error(
    ...
    # Try to access route
    headers = data_generator.auth.get_bearer_header(access_token)
    route_resp = await cli_no_redis.get("/protected_test/second", headers=headers)
    assert route_resp.status_code == 401
